...
func TestAddAndRemoveAttachment(t *testing.T) {
	...
	// Add attachment
	if err := project.AddAttachment(dummyPath); err != nil {
		t.Fatalf("AddAttachment failed: %v", err)
	}

	attachments, err := project.LoadAttachments()
	if err != nil {
	...
	// Remove attachment
	if err := project.RemoveAttachment("dummy.txt"); err != nil {
...
func TestAttachmentErrors(t *testing.T) {
	...
	if err := project.AddAttachment(filepath.Join(tempDir, "does_not_exist.txt")); err == nil {
		t.Error("Expected error when adding non-existent file, got nil")
	}

	// Try to remove a non-existent attachment
	if err := project.RemoveAttachment("notfound.txt"); err == nil {
