...
func (v *ConfigVault) loadTagsIndex() (tagsIndex, error) {
	path := filepath.Join(v.rootPath, snapshotsDir, tagsIndexFile)
	f, err := os.Open(path)
	if err != nil {
...
func (v *ConfigVault) saveTagsIndex(idx tagsIndex) error {
	path := filepath.Join(v.rootPath, snapshotsDir, tagsIndexFile)
...
func (v *ConfigVault) CreateSnapshot(tag, message, author string) (string, error) {
	// Load components
	components, err := LoadComponents(v.GetComponentsPath())
	if err != nil {
	...
	if err := v.SaveSnapshot(snap); err != nil {
		return "", err
	}
	...
	if tag != "" {
		...
		idx[tag] = snapID
		if err := v.saveTagsIndex(idx); err != nil {
			return "", err
		}
	}
	return snapID, nil
...
func (v *ConfigVault) SaveSnapshot(snap Snapshot) error {
	dir := filepath.Join(v.rootPath, snapshotsDir)
...
func (v *ConfigVault) LoadSnapshot(id string) (*Snapshot, error) {
	file := filepath.Join(v.rootPath, snapshotsDir, id+".json")
	f, err := os.Open(file)
	if err != nil {
...
func (v *ConfigVault) ListSnapshots() ([]Snapshot, error) {
	dir := filepath.Join(v.rootPath, snapshotsDir)
